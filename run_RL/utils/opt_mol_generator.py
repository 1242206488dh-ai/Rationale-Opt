...
import torch as th
from utils.maskgnn import collate_molgraphs, EarlyStopping, run_a_train_epoch, \
    run_an_eval_epoch, set_random_seed, RGCN, Meter
import pickle as pkl
from utils.build_data import build_mol_graph_for_one_mol
...
def return_atom_num(smi):
    try:
        sub_mol_atom_num = Chem.MolFromSmiles(smi).GetNumAtoms()
...
def SME_opt_sub_detect(smiles, model_name, rgcn_hidden_feats=[64, 64, 64], ffn_hidden_feats=128,
    ...
    args['lr'] = lr
    args['loop'] = True
    args['task_name'] = model_name

    rgcn_bg, sub_smi_list, smask_idx_list = build_mol_graph_for_one_mol(smiles, for_mol=False)
    if rgcn_bg is None:
        return [-1,-1]
    ...
    for seed in range(1):
        ...
        stopper = EarlyStopping(patience=args['patience'], task_name=args['task_name'] + '_' + str(seed + 1), mode=args['mode'])
        ...
        with th.no_grad():
            ...
            sub_embedding_all, weight = model(rgcn_bg_i, rgcn_node_feats, rgcn_edge_feats, smask_feats, return_embedding=True)
            eval_meter.update(preds, preds)
            th.cuda.empty_cache()

        y_true, y_pred = eval_meter.compute_metric('return_pred_true')
        y_pred = th.sigmoid(y_pred) if args['classification'] else y_pred
        y_pred = y_pred.squeeze().cpu().numpy().tolist()
        if type(y_pred).__name__ != 'list':
            y_pred = [y_pred]
        attri_pred = cal_attri(y_pred)
        sme_opt_detect[f'attri_{seed + 1}'] = attri_pred

    attri_mean = sme_opt_detect[[f'attri_{i + 1}' for i in range(seed + 1)]].mean(axis=1)
    attri_std = sme_opt_detect[[f'attri_{i + 1}' for i in range(seed + 1)]].std(axis=1)
    ...
    filtered = sme_opt_detect_sub[abs(sme_opt_detect_sub['attri_mean']) > abs(sme_opt_detect_sub['attri_std'])]
    if len(filtered) > 0:
        sme_opt_detect = filtered

    sub_smi_list = sme_opt_detect['smiles'].tolist()
    attri_mean = sme_opt_detect['attri_mean'].tolist()
    smi_wt_star = [s.replace("(*)", "").replace("*", "") for s in sub_smi_list]
    smi_he_atom_num = [return_atom_num(s) for s in smi_wt_star]
    atom_num = return_atom_num(smiles)
    change_atom_rate = [n / atom_num for n in smi_he_atom_num]
    sme_opt_detect['change_atom_rate'] = change_atom_rate
    attri_per_he_atom = [attri_mean[i] / smi_he_atom_num[i] for i in range(len(attri_mean))]
    connect_num = [s.count("*") for s in sub_smi_list]
    ...
    sme_opt_detect = sme_opt_detect[sme_opt_detect['change_atom_rate'] <= 0.40]
    ...
    # sme_opt_detect.to_csv('/HOME/scz4306/run/SME/optimization/RL/data/mol_opt_detect_cache.csv')

    if mode == 'higher':
        sme_opt_detect.sort_values(by='attri_per_atom', ascending=True, inplace=True)
    else:
        sme_opt_detect.sort_values(by='attri_per_atom', ascending=False, inplace=True)

    top_n = 2
    if len(sme_opt_detect) == 0:
        print("Warning: No substructures after filtering.")
        return [-1, -1]

    # 如果不足 top_n，则仅处理实际可用的数量
    actual_n = min(len(sme_opt_detect), top_n)
    if actual_n < top_n:
        print(f"Warning: Only {actual_n} substructure(s) available. Proceeding with available ones.")

    results = []

    sub_smi_to_change_list = sme_opt_detect.smiles.tolist()[:actual_n]
    sub_smi_smask_idx_list = sme_opt_detect.smask_idx.tolist()[:actual_n]
    sub_atom_num_list = sme_opt_detect.sub_atom_num.tolist()[:actual_n]
    sub_smi_to_change_value_list = sme_opt_detect.attri_per_atom.tolist()[:actual_n]
    sub_connect_num_list = [s.count("*") for s in sub_smi_to_change_list]

    target_idx_in_all_list = [sub_smi_list.index(smi) for smi in sub_smi_to_change_list]
    sub_smi_embedding_list = [sub_embedding_all[idx].detach().cpu().numpy().flatten() for idx in target_idx_in_all_list]

    for i in range(actual_n):
        context_indices = [j for j in range(target_idx_in_all_list[i] - 2, target_idx_in_all_list[i] + 3)
                        if j != target_idx_in_all_list[i] and 0 <= j < len(sub_embedding_all)]
        context_embeddings = [sub_embedding_all[j].detach().cpu().numpy() for j in context_indices]

        result_i = (
            pred_value,
            sub_smi_to_change_list[i],
            sub_atom_num_list[i],
            sub_smi_smask_idx_list[i],
            sub_smi_to_change_value_list[i],
            sub_connect_num_list[i],
            sub_smi_embedding_list[i],
            context_embeddings
        )
...
#     sme_opt_detect['sub_atom_num'] = smi_he_atom_num
#     sme_opt_detect['connect_num'] = connect_num
#     # sme_opt_detect = sme_opt_detect[sme_opt_detect['change_atom_rate'] <= 0.50]
#     sme_opt_detect.drop_duplicates(subset=['smiles', 'attri_mean'], inplace=True, keep='first')
#     sme_opt_detect = sme_opt_detect[sme_opt_detect['smiles'] != 'NaN']
...
def generate_opt_data(data_name, model_name, sub_name, sample_mol_num, sub_num, mode='higher', random_seed=1216):
    # load hyperparameter
    with open('/HOME/scz4306/run/SME/optimization/result/hyperparameter_{}.pkl'.format(model_name), 'rb') as f:
        hyperparameter = pkl.load(f)
    origin_data = pd.read_csv('/HOME/scz4306/run/SME/optimization/data/origin_data_raw/{}.csv'.format(data_name))
    ...
    opt_data_opt_mol = []
    n_origin_mol = 0
    sub_data = pd.read_csv('/HOME/scz4306/run/SME/optimization/prediction/gain/{}_{}_gain.csv'.format(sub_name, model_name))
