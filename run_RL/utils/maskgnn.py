...
class BaseGNN(nn.Module):
    ...
    def forward(self, rgcn_bg, rgcn_node_feats, rgcn_edge_feats, smask_feats, return_embedding=False):
        # print("输入 rgcn_node_feats:", rgcn_node_feats.shape)
        # print("输入 rgcn_edge_feats:", rgcn_edge_feats.shape)

        for i, rgcn_gnn in enumerate(self.rgcn_gnn_layers):
            rgcn_node_feats = rgcn_gnn(rgcn_bg, rgcn_node_feats, rgcn_edge_feats)
            # print(f"第 {i+1} 层 RGCN 输出 rgcn_node_feats:", rgcn_node_feats.shape)

        graph_feats, weight = self.readout(rgcn_bg, rgcn_node_feats, smask_feats)
        # print("图特征 graph_feats:", graph_feats.shape)
        # print("权重 weight:", weight.shape)

        h1 = self.fc_layers1(graph_feats)
        # print("fc_layers1 输出 h1:", h1.shape)

        h2 = self.fc_layers2(h1)
        # print("fc_layers2 输出 h2:", h2.shape)

        h3 = self.fc_layers3(h2)
        # print("fc_layers3 输出 h3 (embedding):", h3.shape)

        out = self.predict(h3)
        print("预测输出 out:", out.shape)
