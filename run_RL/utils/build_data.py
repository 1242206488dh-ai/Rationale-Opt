...
def build_mol_graph_for_one_mol(smiles, for_mol=False):
    ...
    try:
        ...
        print(f"brics_substructure_dir['substructure']: {brics_substructure_dir['substructure']}")
        print(f"brics_substructure_dir['brics_res_smi']: {brics_substructure_dir['brics_res_smi']}")

        brics_res_smi = brics_substructure_dir['brics_res_smi']

        # 只有当子结构数 > 1 时才认为成功
        if len(brics_res_smi) > 1:
            brics_success = True

        brics_substructure_mask = []
        for idx, substructure in brics_substructure_dir['substructure'].items():
            brics_substructure_mask.append(substructure)
        ...
        for j, smask_i in enumerate(smask):
            if len(smask_i) != mol_number:
                ...
                print(f"分子 {smiles} 的子结构 {brics_res_smi[j]} 成功转换为图。")
            else:
                print(f"跳过第 {j} 个子结构，因为其掩码长度与 mol_number 相等。")
    ...
    try:
        ...
        print(f"[MURCKO] core_smi: {core_smi}")
        print(f"[MURCKO] murcko_res_smi: {murcko_res_smi}")

        # 判断是否有超过一个子结构或至少有一个非核心子结构
        if len(murcko_res_smi) > 1 or any(smi != core_smi for smi in murcko_res_smi):
            murcko_success = True

        for j, smask_i in enumerate(smask):
            if len(smask_i) != mol_number:
                if murcko_res_smi[j] != core_smi:
                    print(f"[MURCKO] Adding murcko substructure: {murcko_res_smi[j]}")
                    g_rgcn_murcko = construct_RGCN_mol_graph_from_smiles(smiles, smask=smask_i)
                    mol_g_list.append(g_rgcn_murcko)
                    smi_sub_list.append(murcko_res_smi[j])
                    smask_idx_list.append(smask_i)
    except Exception as e:
        print(f"[MURCKO ERROR]: {e}")

    # Functional Group (FG) 分解
    try:
        fg_mask = []
        fg_smi = []
        hit_fg_at, hit_fg_smi = return_fg_hit_atom(smiles, fg_smi_list, fg_with_ca_list, fg_without_ca_list)
        print(f"[FG] hit_fg_smi: {hit_fg_smi}")
        ...
        for j, fg_mask_j in enumerate(fg_mask):
            if len(fg_mask_j) != mol_number:
                print(f"[FG] Adding fg substructure: {fg_smi[j]}")
                g_rgcn_fg = construct_RGCN_mol_graph_from_smiles(smiles, smask=fg_mask_j)
    ...
    # 如果三种方法都没有成功碎片化，则跳过
    if not (brics_success or murcko_success or fg_success):
        print(f"跳过分子 {smiles}，因为三种方法都未能碎片化分子。")
