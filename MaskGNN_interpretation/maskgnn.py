...
def collate_molgraphs(data):
    ...
    组合分子图数据，并确保返回值格式正确
    """
    print(f"🔍 Debug: batch data size = {len(data)}")  # 调试: 检查批次大小

    # 确保 data 不是空的
    if len(data) == 0:
        print("❌ Error: Received empty batch data in collate_molgraphs!")
        return None

    try:
        # 解包数据
        (
            smiles, g_rgcn, labels, smask, sub_name,
            antecedents, consequents, support, confidence, 
            lift, leverage, conviction, zhangs_metric, 
            brics_indices, brics_comb_indices,ante_indices, ante_comb_indices,   
            conse_indices, conse_comb_indices ,  
            ante_conse_indices 
        ) = map(list, zip(*data))
    except ValueError as e:
        print(f"❌ Error: zip(*data) failed in collate_molgraphs: {e}")
        print(f"🔹 Debug: data structure {data}")
        return None

    print(f"✅ Debug: Successfully unzipped batch data.")

    try:
        rgcn_bg = dgl.batch(g_rgcn)
    except Exception as e:
        print(f"❌ Error: Failed to batch graphs in collate_molgraphs: {e}")
        return None

    labels = th.tensor(labels)
    
    return (
        smiles, rgcn_bg, labels, smask, sub_name, 
        antecedents, consequents, support, confidence, 
        lift, leverage, conviction, zhangs_metric, 
        brics_indices, brics_comb_indices,ante_indices, ante_comb_indices,   
        conse_indices, conse_comb_indices ,  
...
def run_an_eval_epoch(args, model, data_loader, loss_criterion, out_path, seed=0): 
    ...
    zhangs_metric_list = []
    brics_indices_list = []
    brics_comb_indices_list = []
    ante_indices_list= []
    ante_comb_indices_list= []
    conse_indices_list= []
    conse_comb_indices_list= []
    ante_conse_indices_list= []

    print(f"🚀 开始评估模型: 设备: {args['device']}")

    with th.no_grad():
        if len(data_loader) == 0:
            print("⚠️  警告: data_loader 为空，未加载任何数据!")
            return None, None

        for batch_id, batch_data in enumerate(data_loader):
            print(f"\n🔄 处理 batch {batch_id + 1}/{len(data_loader)} ...")

            try:
                # 解包数据
                (
                    smiles, rgcn_bg, labels, smask_idx, sub_name, 
                    antecedents, consequents, support, confidence, lift, 
                    leverage, conviction, zhangs_metric, brics_indices, brics_comb_indices,
                    ante_indices, ante_comb_indices,  conse_indices, conse_comb_indices ,  ante_conse_indices 
                ) = batch_data
                
                print(f"📌 batch {batch_id}: 解析 {len(smiles)} 个分子")
                ...
                lift_list += lift
                leverage_list += leverage
                conviction_list += conviction
                zhangs_metric_list += zhangs_metric
                brics_indices_list += brics_indices
                brics_comb_indices_list += brics_comb_indices
                ante_indices_list+=ante_indices
                ante_comb_indices_list+=ante_comb_indices 
                conse_indices_list+=conse_indices
                conse_comb_indices_list+= conse_comb_indices 
                ante_conse_indices_list+=ante_conse_indices 

                th.cuda.empty_cache()

            except Exception as e:
                print(f"❌ batch {batch_id} 处理失败: {e}")
        
    print(f"\n📊 评估完成! 总分子数: {n_mol}, 总损失: {total_loss}")

    # 处理除零问题
    if n_mol == 0:
        print("⚠️  警告: n_mol = 0，未计算任何分子数据，可能 data_loader 为空或数据格式错误。")
        return None, None

    average_loss = total_loss / n_mol
    print(f"📉 平均损失: {average_loss:.6f}")
    ...
    if args['classification']:
        y_pred = th.sigmoid(y_pred).squeeze().numpy()
    else:
    ...
    prediction_pd['smarts'] = smask_idx_list
    # prediction_pd['index'] = index_list
    prediction_pd['antecedents'] = antecedents_list
    prediction_pd['consequents'] = consequents_list
    prediction_pd['support'] = support_list
    prediction_pd['confidence'] = confidence_list
    prediction_pd['lift'] = lift_list
    prediction_pd['leverage'] = leverage_list
    prediction_pd['conviction'] = conviction_list
    prediction_pd['zhangs_metric'] = zhangs_metric_list
    prediction_pd['brics_indices'] = brics_indices_list
    prediction_pd['brics_comb_indices'] = brics_comb_indices_list
    prediction_pd['ante_indices'] = ante_indices_list
    prediction_pd['ante_comb_indices'] = ante_comb_indices_list
    prediction_pd['conse_indices'] = conse_indices_list
    prediction_pd['conse_comb_indices'] = conse_comb_indices_list
    prediction_pd['ante_conse_indices'] = ante_conse_indices_list
    ...
    if args['classification']:
        ...
        pre, rec, f1, _ = metrics.precision_recall_fscore_support(y_true_list, y_pred_label, zero_division=0)
        f1, rec, pre = round(f1[1], 4), round(rec[1], 4), round(pre[1], 4)
...
class EarlyStopping(object):
    ...
    def __init__(self, pretrained_model='Null_early_stop.pth', mode='higher', patience=30, filename=None,
                 task_name="None",
                 former_task_name="None"):
        if filename is None:
            task_name = task_name
            filename = '/HOME/scz4306/run/SME/optimization/model/{}_early_stop.pth'.format(task_name)
        former_filename = '/HOME/scz4306/run/SME/optimization/model/{}_early_stop.pth'.format(former_task_name)
        ...
        self.pretrained_model = '/HOME/scz4306/run/SME/optimization/model/' + pretrained_model
    ...
    def load_checkpoint(self, model):
        ...
        checkpoint = th.load(self.filename, map_location=th.device('cpu'))

        # 在加载前打印当前模型的 state_dict
        print("当前模型的 state_dict:")
        for k, v in model.state_dict().items():
            print(k)

        # 在加载后打印保存的 checkpoint 的 state_dict
        print("加载的 checkpoint 的 state_dict:")
